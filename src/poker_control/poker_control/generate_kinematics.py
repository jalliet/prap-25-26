...
def main():
    ...
    joint_2_pos_func = ca.Function(
        'joint_2_pos_func', [q[0]], [joint_positions[1]])

    # Helper for World to Wrist Orientation
    world_to_wrist_func = ca.Function(
        'world_to_wrist_func', [q[0:3]], [joint_3_R])
    ...
    for i in range(6):
        J_p[:, i] = skew_matrix(joint_rotation_axes[i]
                                ) @ (pos_ee - joint_positions[i])
    ...
    p_1_to_wrist_baseFrame = world_to_base @ ca.vertcat(
        world_frame_wrist_position, 1)
    ...
    cos_delta_0 = (lambda_1**2 + lambda_2**2 - dist_reach_0 **
    ...
    p_2_to_wrist_worldFrame = world_frame_wrist_position - \
        joint_2_pos_func(IK[0])
    lambda_3 = ca.norm_2(p_2_to_wrist_worldFrame)

    # Required elbow angle
    cos_ratio = (lambda_1**2 + lambda_2**2 - lambda_3**2) / \
        (2 * lambda_1 * lambda_2)
    ...
    cos_phi_0 = (lambda_1**2 + dist_reach_0**2 - lambda_2**2) / \
        (2 * lambda_1 * dist_reach_0)
